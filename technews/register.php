...
if (isset($_POST["register"])) {
    ...
    if (
        ...
        $error_message =
            "<div class='alert alert-danger'>All fields are required and must be entered</div>";
    } elseif (!preg_match($validIntFormatNumbersRegx, $_POST["phone"])) {
        $error_message =
            "<div class='alert alert-danger'>You have entered an invalid phone number (International Format Only)</div>";
    } else {
        ...
        if (mysqli_num_rows($result) > 0) {
            $error_message =
                "<p style='color:red;text-align:center;margin:10px 0;'>UserName Already Exists</p>";
        } else {
