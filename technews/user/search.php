        ...
        <div class="row">
            <div class="col-md-8">
                <div class="post-container">
                    ...
                    $search_posts_sql = "SELECT post.post_id, post.title, category.category_name,
                    ...
                    ($search_posts_result = mysqli_query($conn, $search_posts_sql)) or die("Query failed: Fetch Search Items");

                    if (mysqli_num_rows($search_posts_result) > 0) {
                        while ($post = mysqli_fetch_assoc($search_posts_result)) {
                    ...
                    <?php
                    // Pagination
                    $search_count_sql = "SELECT COUNT(*) as total FROM post
                                 LEFT JOIN category ON post.category = category.category_id
                                 WHERE post.title       LIKE '%{$search_term}%'
                                    OR post.content     LIKE '%{$search_term}%'
                                    OR post.description LIKE '%{$search_term}%'
                                    OR post.author      LIKE '%{$search_term}%'
                                    OR category.category_name LIKE '%{$search_term}%'";
                    ($search_count_result = mysqli_query($conn, $search_count_sql)) or die("Query Failed");
                    $search_count_row = mysqli_fetch_assoc($search_count_result);

                    if ($search_count_row["total"] > 0) {
                        $total_records = (int)$search_count_row["total"];
