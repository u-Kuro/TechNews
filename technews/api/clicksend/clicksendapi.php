...
if (
    $manualUpdate ||
    $file_not_existing ||
    time() - @filemtime($cacheFile) > $intervalTime
) {
    ...
    if ($shouldRunAPI) {
        ...
        if (mysqli_num_rows($result1) > 0) {
            ...
            $alertMessage =
                '?alertMessage=We\'re sorry, but there are no available phone numbers at this time. This may be because there are currently no users registered on the website or there are no valid international phone numbers. Please try again later or contact support for assistance.';
        }
    }
}
